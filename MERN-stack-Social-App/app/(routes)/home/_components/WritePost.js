...
function WritePost({getAllPost}) {
    ...
    const onCreatePost=()=>{
        if(!userDetail || !userDetail._id){
            toast({
                title: "Please wait",
                description: "Your profile details are still loading. Try again in a moment.",
                variant: "destructive"
            });
            return;
        }
  ...
  return (
    <div>
        <h2 className='text-[30px] font-medium text-gray-600'>Hello,{user?.fullName || 'there'}</h2>
        ...
        rounded-lg mt-5 bg-slate-100'>
            ...
            <div className='mt-2 flex justify-between'>
                ...
                hover:bg-blue-700"
                disabled={!userInputPost?.trim()?.length}
