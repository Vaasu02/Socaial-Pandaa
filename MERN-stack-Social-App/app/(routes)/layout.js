...
function layout({ children }) {
  ...
  const getUserDetails=async()=>{
    ...
    if(!email) return;
    try{
      const resp=await GlobalApi.getUserByEmail(email);
      if(resp?.data){
      ...
      const created=await GlobalApi.createUser(payload);
      setUserDetail(created.data);
    }catch(err){
      // swallow; UI will continue to work but publish will guard
    }
  }
  return (
    <div>
